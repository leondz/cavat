...
#        print elementID, len(self.parsedText), self.parsedText
    ...
    def importCorpusToDb(self,  directory,  dbName):
        ...
        for fileName in fileList:
            ...
            sentenceOffset = {0:0}
            db.cursor.execute('UPDATE sentences SET offset = 0 WHERE doc_id = ? AND sentenceID = 0',  (self.doc_id,))
            sentenceID = 1
            for sentence in sentences:
                ...
                db.cursor.execute('UPDATE sentences SET offset = ? WHERE doc_id = ? AND sentenceID = ?',  (sentenceOffset[sentenceID], self.doc_id,  sentenceID))
                sentenceID += 1
